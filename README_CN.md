...
本项目采用自定义 `Academic Attribution License Version 1.0` 发布，完整条款见 `LICENSE`。

如果在论文、报告、学位论文或其他公开成果中使用本项目的代码、数据、图表或实现思路，请在正文或致谢/引用部分明确声明本仓库，并说明复用的具体内容。

任何使用、再分发或衍生修改均需同时遵守本仓库及相关第三方工具/依赖所对应的开源协议。
